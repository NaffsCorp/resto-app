...
export default function CategoryScreen() {
  ...
  useEffect(() => {
    axios
      .get('http://indo.prinafsika.world:8000/api/size')
      .then((response) => {
  ...
  const addCategory = async () => {
    try {
      ...
      const response = await axios.post(
        'http://indo.prinafsika.world:8000/api/size',
        { nama: form.name },
        {
          headers: {
  ...
  const editCategory = async () => {
    try {
      ...
      if (!form.name) {
        alert('Please fill in the size name')
        return
      }

      const response = await axios.put(
        `http://indo.prinafsika.world:8000/api/size/${editingId}`,
      ...
      alert(error.response?.data?.message || 'Error updating size')
    }
  }
  ...
  const deleteCategory = async (id) => {
    try {
      ...
      await axios.delete(`http://indo.prinafsika.world:8000/api/size/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
